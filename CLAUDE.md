...
**Auth:** `go-pkgz/auth` with Google OAuth + an env-credential fallback (`internal/app/web/auth`); JWT-protected `/api/v1/*` routes; personal access tokens (`wfp_…`, SHA-256-hashed at rest, `pkg/pat`) accepted as `Bearer` (via `internal/app/web/middleware/patauth`) or Basic-auth password, managed at `/user/tokens`; mTLS for hub↔agent.
